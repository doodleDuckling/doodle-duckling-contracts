...
contract DoodleDucklingStamp3 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function updateSpecialClaimCardRules(uint[] memory ruleIds, uint[] memory startTimes, uint[] memory endTimes, uint[] memory stampIds, bytes32[] memory merkleRoots,bool[] memory everyoneCanClaims) public onlyOwner {
        for (uint i = 0; i < ruleIds.length; i++) {
            specialClaimCardRuleList[ruleIds[i]] = SpecialClaimCardRule(startTimes[i], endTimes[i], stampIds[i], merkleRoots[i],everyoneCanClaims[i]);
        }
    }
    ...
    function claimCardWithSpecialRule(uint ruleId, bytes32[] memory proof) public onlyOpened {
        ...
        require(specialClaimCardRule.everyoneCanClaim || checkWhitelist(proof, specialClaimCardRule.merkleRoot), 'ec10');
